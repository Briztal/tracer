...
/*
 * The memory map describes as simply as possible a succession of equally spaced memory blocs;
 *
 * 	Is is made to contain as less information as possible, to be passed as argument to transfer functions.
 *
 * 	For this reason, the size of a bloc and the number of available blocs, that are information that
...
/*
 * The block descriptor contains the size of a block, and the number of blocs;
 *
...
struct blocks_desc_t {
	...
	//The current number of available blocs in the memory zone;
	size_t nb_blocks;
...
/*
 * The memory descriptor : describes completely a succession of memory blocs.
 *
...
//Transfer the maximal amount of memory from a memory descriptor to another; Returns the number of copied blocs;
