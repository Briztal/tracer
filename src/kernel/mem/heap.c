...
static void heap_check_block(const struct heap_head *const heap, const struct heap_block *const block) {
	...
	if ((status != HEAP_ALLOCATED_STATUS) && (status != HEAP_FREE_STATUS)) {
		...
		//Log and quit;
		kernel_panic("heap.c : heap_free : block status invalid. memory leak;");
...
void *heap_malloc(struct heap_head *heap, size_t size) {
	...
	if (!first_block) {
		...
		//Fail, no space left;
		kernel_panic("heap.c : heap_malloc : no space left");
	...
	kernel_panic("heap.c : heap_malloc : no memory block large enough;");
