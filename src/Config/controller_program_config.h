 ...
 *
 * ALLOW_MULTIPLICITY(count, name) : tells the high level that last 'count' data (declared using REQUIRE_ARGS)
 *      can be sent multiple times (ex : to enqueue_movement on several axis, [AXIS_ID, VALUE] data will be sent for each concerned axis.
 *
 *  -----------------------------------------------------------------------------------------------------------------
 *
