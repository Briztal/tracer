...
template<typename T>
Array<T>::Array(uint8_t size) : max_size(size), buffer(new T[size]), data_pointer(buffer), nb_elements(0),
                                  nb_spaces(size) {
...
template<typename T>
bool Array<T>::safeAdd(const T &data) {
