...
float JerkPlanner::get_jerk_point_speed(const float *initial_steps_distances, const uint8_t speed_group,
    ...
    for (uint8_t trajectory_control = 0; trajectory_control < NB_STEPPERS; trajectory_control++) {
        ...
        //Formula : regulation_speed(unit/s) < max_jerk(feedrate/s) / abs( final_ratio(feedrate/unit) - initial_ratio(feedrate/unit) ).
        stepper_data_t *data = SteppersData::steppers_data + trajectory_control;
        float max_speed = data->jerk * data->feedrate / algebraic_difference;
...
/*
 ...
 FORMULAS FOR JERK REGULATION :
    ...
    Notations :
        ...
        - vi- (feedrate/s) : trajectory_control motor i speed before the movement (feedrate/s);
        - vi+ (feedrate/s) : trajectory_control motor i speed after the movement (feedrate/s);
        - si- (feedrate)   : trajectory_control motor i distance for the sub movement before the jerk point;
        - si+ (feedrate)   : trajectory_control motor i distance for the sub movement after the jerk point;
        - Ji (feedrate/s)  : jerk limit on trajectory_control i;
