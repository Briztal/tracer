...
#include "StepperControl/trajectory_control.h"
...
void HomingMovement::prepare_movement(sig_t reset, uint8_t *endstops, sig_t directions) {
    ...
    for (int axis = 0; axis < NB_STEPPERS; axis++) {
        ...
        delays[axis] = (uint32_t) (1000000 / (data->jerk * data->feedrate));
    }
    ...
    /*
     * trajectory_control config :
     */

    //Set the trajectory_control directions;
    trajectory_control::set_directions(directions);

    //Enable all trajectory_control
    trajectory_control::enable(255);
...
/*
 * phase_1 : this function feedrate all remaining axis, and eventually updates the signature and sleep.
 *
...
void HomingMovement::phase_1() {
    ...
    //Step;
    trajectory_control::fastStep(movement_signature);

    //If we made [step] feedrate since last update :
    if (!--step_index) {
