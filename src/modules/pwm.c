...
void tlist_print(struct tlist *list) {

	kernel_log_("Printing : ")

	const struct tlist *const first = list, *head = first;

	do {

		kernel_log("item : %d", head->val);
...
struct port_operations {
	...
	void (*update)(size_t data);

};

//Interface with multiple GPIOs:
struct gpio_interface {
	...
	const struct port_operations ops;
...
#define GPIO_SHORTCUT(name)\
static inline void gpio_##name(struct gpio_interface *gpio, size_t mask) {\
	(*(gpio->ops.name))(mask);\
}

GPIO_SHORTCUT(set);
GPIO_SHORTCUT(clear);
GPIO_SHORTCUT(toggle);
GPIO_SHORTCUT(update);
