...
void JerkPlanner::compute_jerk(const movement_data_t *current_movement, movement_data_t *previous_movement) {
    ...
    //Declare an array that will contain the machine_control positions;
    float stepper_positions[NB_STEPPERS];
    ...
    for (uint8_t machine_control = 0; machine_control < NB_STEPPERS; machine_control++) {
        jerk_pos[machine_control] = (int32_t) stepper_positions[machine_control];
    }
...
void JerkPlanner::get_ending_jerk_ratios(float *final_steps_distances, const uint8_t speed_group,
    ...
    for (uint8_t machine_control = 0; machine_control < NB_STEPPERS; machine_control++) {
        final_jerk_ratios[machine_control] = final_steps_distances[machine_control] / distance_inverse;
    }

}

/*
 * get_stepper_distances : this function gets the algebraic step_distances on each machine_control, between the two provided
...
void JerkPlanner::get_stepper_distances(const float p0, const float p1, void (*trajectory_function)(float, float *),
    ...
    //Get the machine_control position for the point p0;
    StepperController::get_stepper_positions_for(trajectory_function, p0, t0);

    //Get the machine_control position for the point p1;
    StepperController::get_stepper_positions_for(trajectory_function, p1, t1);

    //Get distances;
    for (uint8_t machine_control = 0; machine_control < NB_STEPPERS; machine_control++) {
        distances[machine_control] = t1[machine_control] - t0[machine_control];
    }
...
float JerkPlanner::get_jerk_point_speed(const float *initial_steps_distances, const uint8_t speed_group,
    ...
    for (uint8_t machine_control = 0; machine_control < NB_STEPPERS; machine_control++) {
        ...
        //determine the initial jerk_ratio for the current machine_control;
        float initial_jerk_ratio = initial_steps_distances[machine_control] * distance_inverse;

        //determine the algebric ratio difference;
        float algebraic_difference = final_jerk_ratios[machine_control] - initial_jerk_ratio;
        ...
        stepper_data_t *data = SteppersData::steppers_data + machine_control;
...
/*
 ...
 FORMULAS FOR JERK REGULATION :
    ...
    Definition 1 : global distance D. The global distance during a movement is the distance that the machine on
     ...
     Definition 2 : jerk limit Ji. A machine_control mustn't have its speed changing more than its jerk limit in a brief time
        period.
    ...
    Notations :
        ...
        - vi- (steps_per_unit/s) : machine_control motor i speed before the movement (steps_per_unit/s);
        - vi+ (steps_per_unit/s) : machine_control motor i speed after the movement (steps_per_unit/s);
        - si- (steps_per_unit)   : machine_control motor i distance for the sub movement before the jerk point;
        - si+ (steps_per_unit)   : machine_control motor i distance for the sub movement after the jerk point;
        - Ji (steps_per_unit/s)  : jerk limit on machine_control i;
    ...
    Proposition 1 : The global speed before a jerk point and after a jerk point is the same.
        ...
        Explanation : The speed variation is regulated by the machine, and can only occur continuously (or, as the
            ...
            This means that only machine_control speeds will vary during the jerk.
    ...
    Definition 3 : targetVector speed. The targetVector speed Vt, for a jerk point, is the maximum speed that allow all machine_control to
        stay beyond their jerk limit.


    Determination of Vt :

        For all machine_control, we must have
    ...
    Result :
        ...
        Vt is the greater speed that verifies for all machine_control the following condition :
