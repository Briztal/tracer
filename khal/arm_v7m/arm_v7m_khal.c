...
/*The function that shoudl be executed when the armv7m is initialised.*/
void arm_v7m_exec();
...
static void
(*dynamic_vtable[NVIC_NB_EXCEPTIONS])() __attribute__ ((aligned (512)));
...
void
(*static_vtable[NVIC_NB_EXCEPTIONS])()  __attribute__ ((section(".vectors"))) = {
...
void *__cpu_create_stack_context(void *sp_reset, void (*function)(),
...
static void log_BFSR(const uint8_t BFSR, const uint32_t BFAR)
{
	...
	if (BFSR & BFSR_PRECISERR) {
		...
		if (BFSR & MMFSR_MMARKVALID) {
			...
			/*Log;*/
			__printkf("Bus precise data access error : %h",
					  (const void **) BFAR, 1);
...
static void log_UFSR(const uint16_t UFSR)
{
	...
	LOG_FAULT(UFSR, UFSR_INVSTATE,
			  "invalid EPSR.T bit or illegal EPSR.IT bits");
...
static void log_HFSR(const uint32_t HFSR)
{
	...
	/*vector table read error;*/
	LOG_FAULT(HFSR, HFSR_FORCED,
			  "Configurable fault disabled, escalated in hardfault");
