...
#include <EEPROMMap/EEPROMStorage.h>
#include <interface.h>
#include <StepperControl/Machine.h>
#include <EEPROMMap/_eeprom_storage_data.h>
#include <EEPROMMap/EEPROMInterface.h>
...
task_state_t ProgramInterfaceCommands::actions_system_canal(char *data, uint8_t size) {
    ...
    switch (sub_canal) {
        case 0 ://The initialisation case : send_packet a packet for each machine_control
...
task_state_t ProgramInterfaceCommands::EEPROM_system_canal(char *data, uint8_t size) {
    ...
    switch (sub_canal) {
        ...
        case 1 : //Read case
            f = 0;//EEPROMMap::read_integer(data, size);
